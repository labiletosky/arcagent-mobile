...
<div class="grid">
  ...
  <div class="card">
    ...
    <div class="help-box notice-box">
      <p class="helper-text notice-text">If agent doesn't execute within 24 hours, you can claim a full refund.</p>
    </div>
...
list.innerHTML += `<div class="order-item"> <div class="order-id">#${Number(o.id)}</div> <div class="order-details"> <div class="order-item-name"> ${itemName} <span style="font-size:9px;padding:2px 8px;border-radius:4px;background:${catColor}22;color:${catColor};border:1px solid ${catColor}44;margin-left:6px;letter-spacing:1px;text-transform:uppercase;">${category}</span> </div> <div class="order-meta">${amt} USDC · ${o.buyer.slice(0, 6)}...${o.buyer.slice(-4)}</div> </div> <div class="order-status ${statusClass}">${statusLabel}</div> </div>`
}
} catch (e) {
...
setTimeout(() => { document.getElementById(‘copyOrderIdBtn’).textContent = ‘Copy Order ID’ }, 1500)
} catch { document.getElementById(‘copyOrderIdBtn’).textContent = ‘Copy failed’ }
})
